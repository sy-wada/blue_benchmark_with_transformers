...
class Sharding:
    def __init__(self, input_files, output_name_prefix, n_shards):
        assert len(input_files) > 0, 'The input file list must contain at least one file.'
        assert n_shards > 0, 'There must be at least one output shard.'

        self.n_training_shards = n_shards
    ...
    def segment_articles_into_sentences(self, segmenter):
        ...
        if use_multiprocessing == 'manager':
            ...
            jobs = []
            # in addition to the main process, total = n_proc+1
    ...
    def init_output_files(self):
        ...
        for i in range(self.n_training_shards):
            name = self.output_name_prefix + '_' + str(i) + self.output_file_extension
    ...
    def distribute_articles_over_shards(self):
        ...
        n_sentences_assigned_to_training = total_sentences
...
class NLTKSegmenter:
    ...
    def segment_string(self, article):
        return self.nltk.tokenize.sent_tokenize(article)
